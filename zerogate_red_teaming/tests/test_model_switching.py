...
from zerogate_red_teaming import constants as cs
from zerogate_red_teaming import exceptions as ex
from zerogate_red_teaming.config import ModelConfig
from zerogate_red_teaming.main import _create_model_from_string, _handle_model_command
...
def mock_console() -> Generator[MagicMock]:
    with patch("zerogate_red_teaming.main.app_context") as mock_ctx:
...
def mock_settings() -> Generator[MagicMock]:
    with patch("zerogate_red_teaming.main.settings") as mock_s:
...
class TestHandleModelCommand:
    ...
    def test_switch_to_new_model(
        ...
        with (
            patch("zerogate_red_teaming.main.logger") as mock_logger,
            patch(
                "zerogate_red_teaming.main._create_model_from_string",
    ...
    def test_switch_model_with_extra_whitespace(
        ...
        mock_new_config = MagicMock()
        with (
            patch("zerogate_red_teaming.main.logger"),
            patch(
                "zerogate_red_teaming.main._create_model_from_string",
                return_value=(
                    mock_new_model,
...
class TestModelOverrideInAgentLoop:
    ...
    async def test_model_override_passed_to_agent_run(self) -> None:
        from zerogate_red_teaming.main import _run_agent_response_loop
        from zerogate_red_teaming.types_defs import CHAT_LOOP_UI, ConfirmationToolNames
        ...
        with (
            patch("zerogate_red_teaming.main.app_context") as mock_ctx,
            patch("zerogate_red_teaming.main.log_session_event"),
        ):
...
class TestMultipleModelSwitches:
    def test_multiple_switches_in_sequence(
        ...
        with (
            patch("zerogate_red_teaming.main.logger"),
            patch("zerogate_red_teaming.main._create_model_from_string") as mock_create,
        ):
