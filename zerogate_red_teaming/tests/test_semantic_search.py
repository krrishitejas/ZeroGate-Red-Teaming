...
from zerogate_red_teaming.utils.dependencies import has_semantic_dependencies
...
def test_semantic_code_search_returns_empty_without_dependencies() -> None:
    ...
    from zerogate_red_teaming.tools.semantic_search import semantic_code_search
...
def test_semantic_code_search_returns_empty_when_no_matches(
    ...
    with (
        patch("zerogate_red_teaming.embedder.embed_code", mock_embed_code),
        patch("zerogate_red_teaming.vector_store.search_embeddings", mock_search_empty),
    ):
...
def test_semantic_code_search_handles_exception(mock_embed_code: MagicMock) -> None:
    ...
    with patch("zerogate_red_teaming.embedder.embed_code", mock_embed_code):
...
def test_semantic_code_search_preserves_score_order(
    ...
    with (
        patch("zerogate_red_teaming.embedder.embed_code", mock_embed_code),
        patch("zerogate_red_teaming.vector_store.search_embeddings", mock_search),
        patch(
            "zerogate_red_teaming.services.graph_service.MemgraphIngestor",
            return_value=mock_ingestor,
        ),
...
def test_get_function_source_code_returns_source(mock_ingestor: MagicMock) -> None:
    from zerogate_red_teaming.tools.semantic_search import get_function_source_code
    ...
    with (
        ...
        patch(
            "zerogate_red_teaming.utils.source_extraction.validate_source_location",
            mock_validate,
        ),
        patch(
...
def test_create_semantic_search_tool_returns_tool() -> None:
    ...
    from zerogate_red_teaming.tools.semantic_search import create_semantic_search_tool
