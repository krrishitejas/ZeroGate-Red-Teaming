...
from zerogate_red_teaming.utils.dependencies import has_qdrant_client

pytestmark = pytest.mark.skipif(
    not has_qdrant_client(), reason="qdrant-client not installed"
)

_PATCH_CLIENT = "zerogate_red_teaming.vector_store.get_qdrant_client"
_PATCH_SLEEP = "zerogate_red_teaming.vector_store.time.sleep"


class TestUpsertWithRetry:
    def test_succeeds_on_first_attempt(self) -> None:
        from zerogate_red_teaming.vector_store import _upsert_with_retry
...
class TestStoreEmbeddingBatch:
    def test_returns_count_on_success(self) -> None:
        from zerogate_red_teaming.vector_store import store_embedding_batch
...
class TestVerifyStoredIds:
    def test_returns_found_ids(self) -> None:
        from zerogate_red_teaming.vector_store import verify_stored_ids
