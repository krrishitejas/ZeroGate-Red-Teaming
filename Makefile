...
typecheck: ## Run type checking with ty
	$(PYTHON) ty check -v --exclude zerogate_red_teaming/tests/
...
pre-commit: ## Run all pre-commit checks locally (comprehensive test before commit)
	...
	$(PYTHON) ty check --exclude zerogate_red_teaming/tests/
	...
	$(PYTHON) bandit -c pyproject.toml --severity-level high -r zerogate_red_teaming/ --exclude zerogate_red_teaming/tests/,scripts/
